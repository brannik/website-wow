    ...
    public function __construct()
    {
        try
        {
            ...
            if ($this -> validateInput())
            {
				...
				$acc_id = 0;
				$sql = "SELECT id FROM account WHERE username='" . $acc_name. "'";
				$result = $conn->query($sql);
				if($result){
				foreach($result as $row10){
					$acc_id = $row10["id"];
				}
				$conn->close();
				$conn2 = new mysqli($servername, $username, $password, "site");
				//$sql2 = "INSERT INTO messages (autor,reciever,title,text) VALUES ('".$autor."','".$reciever."','".$_POST["msgTitle"]."','".$_POST["msgText"]. "')";
			
				$sql2 = "INSERT INTO account (id,username) VALUES('".$acc_id."','".$acc_name."')"; 
				$result22 = $conn2->query($sql2);
				if($result22){
					echo'DONE';
				}
				...
				$conn2->close();
				echo'SAVED';
				echo'<text>ID: '.$acc_id.'</text>';
				echo'<text>ACC: '.$_POST['accountname'].'</text>';
				echo'<text>PASS: '.$_POST['password'].'</text>';
			}
			else{
				echo'Conn error';
			}
